...
import com.m1guelsb.springauth.dtos.ChangeDTO;
import com.m1guelsb.springauth.dtos.UserDTO;
...
import com.m1guelsb.springauth.exceptions.*;
import com.m1guelsb.springauth.exceptions.LastDataException;
import com.m1guelsb.springauth.exceptions.NoExistDataException;
import com.m1guelsb.springauth.exceptions.NoTargetUserException;
import com.m1guelsb.springauth.exceptions.NotEnougnAmountException;
import com.m1guelsb.springauth.exceptions.RepeatedDataException;
...
public class UserController {
  ...
  public ResponseEntity<String> transfer(@RequestHeader("Authorization") String bearerToken,@RequestParam String phone, @RequestParam int money) {
    if (bearerToken != null) {
...
//  }
  ...
  @Operation(summary = "My endpoint", security = @SecurityRequirement(name = "bearerAuth"))
  public ResponseEntity<String> addEmail(@RequestHeader("Authorization") String bearerToken, @RequestBody String newEmail) {
    if (bearerToken != null) {
      var login = tokenService.validateToken(bearerToken.replace("Bearer ", ""));
      var user = (User) userRepository.findByLogin(login);
//      log.info("/////////////////////////////////////////////////// "+ user.getLogin() + "   " + user.getPassword() + " " + user.getEmailList().isEmpty() );
//      log.info(Arrays.toString(user.getEmailList().toArray())) ;
...
//      log.info(Arrays.toString(strEmailsList.toArray())) ;
  ...
  @Operation(summary = "My endpoint", security = @SecurityRequirement(name = "bearerAuth"))
  public ResponseEntity<String> addPhone(@RequestHeader("Authorization") String bearerToken, @RequestBody String newPhone) {
    if (bearerToken != null) {
      var login = tokenService.validateToken(bearerToken.replace("Bearer ", ""));
      var user = (User) userRepository.findByLogin(login);
  ...
  @Operation(summary = "My endpoint", security = @SecurityRequirement(name = "bearerAuth"))
  public ResponseEntity<String> deletePhone(@RequestHeader("Authorization") String bearerToken, @PathVariable String phone) {
    if (bearerToken != null) {
      var login = tokenService.validateToken(bearerToken.replace("Bearer ", ""));
      var user = (User) userRepository.findByLogin(login);
  ...
  @Operation(summary = "My endpoint", security = @SecurityRequirement(name = "bearerAuth"))
  public ResponseEntity<String> deleteEmail(@RequestHeader("Authorization") String bearerToken, @PathVariable String email) {
    if (bearerToken != null) {
      var login = tokenService.validateToken(bearerToken.replace("Bearer ", ""));
      var user = (User) userRepository.findByLogin(login);
  ...
  @Operation(summary = "My endpoint", security = @SecurityRequirement(name = "bearerAuth"))
  public ResponseEntity<String> changePhone(@RequestHeader("Authorization") String bearerToken, @RequestBody ChangeDTO newAndOldPhone) {
    if (bearerToken != null) {
      var login = tokenService.validateToken(bearerToken.replace("Bearer ", ""));
      var user = (User) userRepository.findByLogin(login);
      log.info("/////////////////////////////////////////////////// "+ user.getLogin() + "   " + user.getPassword() + " " + user.getPhoneList().isEmpty() );
  ...
  @Operation(summary = "My endpoint", security = @SecurityRequirement(name = "bearerAuth"))
  public ResponseEntity<String> changeEmail(@RequestHeader("Authorization") String bearerToken, @RequestBody ChangeDTO newAndOldEmail) {
    if (bearerToken != null) {
      var login = tokenService.validateToken(bearerToken.replace("Bearer ", ""));
      var user = (User) userRepository.findByLogin(login);
     // log.info("/////////////////////////////////////////////////// "+ user.getLogin() + "   " + user.getPassword() + " " + user.getPhoneList().isEmpty() );
